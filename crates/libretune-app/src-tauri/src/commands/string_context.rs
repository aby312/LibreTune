...
pub async fn build_string_context_filtered(
    ...
    // with `current_tune` held deadlocked those writes — they hold the
    // connection and then take `current_tune` — for as long as nothing tore
    // one side down (the W2 ~90 s stall). A busy connection mutex means
    // serial I/O is in flight, which is itself proof of "online".
